...
- Existing Contacts already production: one-entity-per-person model, teamInboxIds derivation, Agent-denied access.

# Canonical Business Rules

- One Global Contact per unique identifier per workspace.
- Contact Area Context separate record (contactId, area, teamInboxId, status, source).
...
- Contact visibility evaluation order: module permission → area scope → visibility scope → team inbox → data privacy masking.
...
- Conversation/Ticket contact linking resolves to existing Global Contact. Creates Area Context if missing.
...
- User assigned to conversation/ticket may see contact summary even if centralized Contacts would not show contact.
- Opening full Contact Detail from assigned conversation still enforces Contacts permission.

# Critical Dependencies

- Existing Contacts data model (teamInboxIds field must migrate to Contact References).
...
- Legacy contacts without Team Inbox scope become invisible (zombie records).
- Migration `SHOULD` default mapping creates inconsistency (workspaces behave differently).
- Duplicate handling change API breaking — all existing clients must update.
...
- Broadcast drafts referencing contacts invisible after update.
...
- Contact blocked but has active Area Context: visibility still allowed, Broadcast disabled.
...
- Update does not invalidate existing Contacts PRD — layers on top.
- Migration default mapping must use `MUST` not `SHOULD` for consistency.

# Open Questions

- What happens to Area Context when conversation moves between Operational and Sales Team Inbox?
- Exact field boundary between "contact summary" and "contact detail" for FR-060/FR-061?
- How Contact References backfilled for existing conversations/tickets?
- What happens to Broadcast drafts referencing contacts no longer visible due to area mismatch?
- Should legacy contacts without Team Inbox scope get system-level Area Context to remain visible?
