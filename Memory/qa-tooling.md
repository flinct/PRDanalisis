...
| **Per-device runner** | ✅ | `runner.js` (zero-dep, port 9876) dijalankan di tiap PC tester → Playwright jalan di PC itu pakai repo lokalnya, host tidak terbebani. ⚙ Settings §5 Execution: pilih "host" vs "perangkat ini" (localStorage per-device). **Arsitektur polling** (bukan SSE stream): POST `/run` → spawn background → `{runId}`; dashboard POLL `GET /run/:id?since=N`. Proses TIDAK dibunuh saat koneksi putus (fix: streaming lintas-origin LAN→localhost putus ~7ms karena Chrome PNA). Run pakai `node node_modules/@playwright/test/cli.js` (bukan npx → hindari quoting Windows) + `--project=chromium`. Spec map presisi via `buildSpecIndex` (scan `[ID]` di judul test). Setup: `RUNNER-SETUP.md`. |
