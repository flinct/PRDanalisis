...
|---|---|---|---|
| v1.0 | 2026-06-22 | Hermes | Initial PRD for phased superAdmin company list/search, onboarding approval operations, tenant impersonation read-only mode, audit log, and controlled write mode. |
...
| Scope | This PRD defines a phased delivery: phase 1 company list/search and onboarding approval, phase 2 tenant impersonation in read-only mode with audit log, and phase 3 controlled write mode with quick admin actions such as change permission. |
| Key Capabilities | Central company list, owner visibility, new company highlighting, approval workflow, tenant impersonation, visible tenant banner, read-only enforcement, audit logging, and future scoped write actions. |
| Outcome | SatuInbox gains a real superAdmin operational lane that works with current tenant architecture rather than bypassing it unsafely. |
...
| Phase | Scope |
|---|---|
| Phase 1 | Company list/search, new company badge/banner, owner user displayed at company list, approve/reject onboarding company. |
| Phase 2 | Tenant impersonation / switch context, read-only mode, visible tenant banner, audit log for access. |
| Phase 3 | Controlled write mode, quick admin actions such as change permission, with stronger audit and explicit write guardrails. |
...
| Phase 2 Impersonation | **FR-015 [P1]**: System MUST support superAdmin tenant impersonation / switch context into a chosen company and organization. **FR-016 [P1]**: Impersonation MUST preserve the existing single-tenant downstream runtime model. **FR-017 [P1]**: Impersonation session MUST carry actor metadata identifying the real `SUPER_ADMIN` and the impersonated target tenant. |
| Read-Only Enforcement | **FR-018 [P1]**: Phase 2 impersonation MUST default to read-only mode. **FR-019 [P1]**: In read-only mode, downstream write-capable actions MUST be blocked either by backend guard, frontend guard, or both. **FR-020 [P1]**: The UI MUST visibly show read-only impersonation mode. |
| Audit and Observability | **FR-021 [P0]**: Company approval and rejection actions MUST be audited. **FR-022 [P1]**: Impersonation start and end MUST be audited. **FR-023 [P2]**: Write-mode actions MUST be audited with before/after values and actor metadata. |
| Phase 3 Write Mode | **FR-024 [P2]**: System MAY provide an explicit mode switch from read-only to write mode for approved quick actions only. **FR-025 [P2]**: Write mode MUST be opt-in, explicit, and visually distinct from read-only mode. **FR-026 [P2]**: Quick admin actions MUST be allowlisted. **FR-027 [P2]**: `Change permission` MUST be treated as a high-risk quick action requiring explicit audit coverage. |
| Non-Side Effects | **FR-028 [P0]**: Super-admin company list MUST NOT alter normal tenant user sessions. **FR-029 [P1]**: Impersonation MUST NOT remove tenant scoping from downstream services. **FR-030 [P2]**: Write mode MUST NOT silently grant unrestricted mutation rights beyond the allowlisted action set. |
| SuperAdmin Login Verification (2FA) | **FR-031 [P0]**: SuperAdmin login MUST require a one-time email verification code as a second factor after password validation. **FR-032 [P0]**: The code MUST be sent to the superAdmin's registered email using the configured `MAIL_FROM` sender. **FR-033 [P0]**: The code MUST remain valid for 24 hours. **FR-034 [P0]**: If a valid code already exists within its window, the system MUST resend the same code rather than generate a new one. **FR-035 [P0]**: A platform-admin session MUST NOT be issued until the code is verified. **FR-036 [P1]**: Code verification attempts MUST be rate-limited to resist brute force. |
...
| Limitation | Impact |
|---|---|
| Phase 1 does not include impersonation. | SuperAdmin can review and approve companies but cannot inspect tenant UI yet. |
| Phase 2 is read-only only. | Support actions requiring mutation remain unavailable until later phase. |
| Phase 3 write mode is allowlisted, not global. | SuperAdmin cannot mutate arbitrary module data by default. |
...
| Open Questions | Exact owner data fields shown in company list, explicit default organization selection rule, and final phase-3 quick-action allowlist. |
