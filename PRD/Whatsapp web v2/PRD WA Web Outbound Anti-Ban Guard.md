...
| Outbound Attempt | `VALIDATING` | Protected resend condition detected in warning mode | `WARNED_RESEND` | System | Classifier result indicates protected resend and mode = warning_only | Warning prepared; no internal formula exposed | `wa.outbound.resend.warning` |
...
| Outbound Attempt | `VALIDATING` | Protected resend condition detected in queue mode | `QUEUED_RESEND` | System | Classifier result indicates protected resend and mode = queue_only | Cooldown/queue timer started; send not dispatched immediately | `wa.outbound.resend.queued` |
