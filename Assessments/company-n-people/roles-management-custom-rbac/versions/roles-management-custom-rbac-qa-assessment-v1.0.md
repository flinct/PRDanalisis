...
> **Reference Memory:** `Memory/global-memory.md`, `Memory/contact-context-visibility.md`, `Memory/CLAUDE-be.md`, `Memory/CLAUDE-fe.md`  
...
- Initial version.
- Menetapkan bahwa feature ini berdampak lintas-domain dan lintas-service, bukan sekadar halaman settings.
- Keputusan awal: **REVISE_PRD** sebelum aman dibawa ke versi berikutnya.
...
**Business Context:**  
Current product sudah punya role-sensitive behavior di Conversation, Ticket, Contacts, Analytics, Broadcast, Leads, Team Inbox, dan Settings. Karena itu perubahan RBAC editor akan mempengaruhi **runtime authorization**, **data visibility**, **masking**, **session refresh**, dan **auditability** di banyak modul sekaligus.
...
**Decision Statement:**
> Arah feature ini benar dan bernilai tinggi, tetapi belum aman untuk langsung dibawa ke versi berikutnya karena masih ada gap mendasar pada contract enforcement, migration, scope resolution, session invalidation, dan privacy masking lintas-surface. Tanpa penguncian kontrak tersebut, risiko access leak, stale permission, dan regression lintas Conversation/Ticket/Contacts/Analytics terlalu besar.
...
- [ ] Tambahkan **API / event contract** untuk role CRUD, role assignment change, audit emission, session refresh/invalidation, dan permission cache invalidation.
...
- PRD belum mendefinisikan failure mode untuk permission mapping drift dan dependency rule drift.

### 2.4 Complexity and Risk Snapshot

- **Complexity Level:** Critical
- **Risk Level:** Critical
- **Primary Impact Areas:** Backend / API / UI / RBAC / Database / Analytics / Contacts / Ticket / Conversation / Audit / Migration / Integration
...
| BR-08 | Semua mutasi role harus tercatat di audit log. | US-013; FR-054–FR-056 |
...
- Perubahan role diterapkan ke assigned members dengan latency yang terdefinisi.
...
- Conversation, Ticket, Contacts, Analytics, Broadcast, Leads, dan Settings sudah punya scope/permission behavior masing-masing.
...
- **Data privacy** tidak boleh berhenti di FE label; harus ikut mempengaruhi source response, export, search hit rendering, dan audit visibility.
...
| Session/Auth | Permission refresh / invalidation | Auth/session | Role -> Session | Harus lock apakah token refresh atau force logout |
...
- Agent memperoleh akses data client full karena masking leak di export/API.
...
- **Monitoring / Alerting Needs:** unauthorized spikes, 403 spikes, masking mismatch alerts, role save failure, permission resolution errors.
- **Logging / Audit Gaps:** perlu log structured untuk role diff, affected member count, session invalidation result, permission resolver fallback.
...
- Gap paling kritis ada di permission enforcement, migration, privacy, dan consistency lintas service/surface.
...
| Required Revisions | Tambahkan API/event contract, migration plan, session refresh policy, privacy enforcement contract, scope resolver truth table, dan metadata correction |
| Suggested Delivery Strategy | Split menjadi Phase 1A (role editor + stable mapping + protected defaults) dan Phase 1B (expanded scope models + advanced exclusions + broader masking surfaces) |
...
| FR-039–FR-045 | Dependency rules | Rule belum cukup lengkap untuk semua queue/team/assigned permutations | Validation / Ticket / Conversation | Pending | Open |
